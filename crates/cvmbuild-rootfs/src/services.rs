...
pub fn apply_services(rootfs: &Path, config: &Config) -> Result<()> {
    ...
    // /etc/resolv.conf — bake in public DNS at build time. networkd
    // tracks DHCP-supplied DNS servers internally (`networkctl status`
    // shows them) but does NOT update /etc/resolv.conf without
    // systemd-resolved, which we don't ship to keep the rootfs small.
    // Image authors who need a different resolver overlay this file
    // in their cvm.toml's [overlay]. Inference images that block
    // outbound at the firewall don't actually use these servers.
    write_rootfs_file(
        rootfs,
        "etc/resolv.conf",
        "# Generated by cvmbuild. Override via [overlay] in cvm.toml.\nnameserver 1.1.1.1\nnameserver 8.8.8.8\n",
    )?;
